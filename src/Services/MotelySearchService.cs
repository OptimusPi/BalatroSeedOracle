...
public class MotelySearchService : IDisposable
{
    ...
    {
        _historyService =
            ServiceHelper.GetService<SearchHistoryService>() ?? new SearchHistoryService();
    }
    ...
    public async Task<(
        ...
        string? author,
        string? description
    )> LoadConfigAsync(string configPath)
    {
        ...
        try
        {
            ...
            return (
                true,
                $"Loaded config with {must} must, {should} should, {mustNot} mustNot clauses",
                _currentConfig.Name,
    ...
    )
    {
        ...
        try
        {
            ...
            _resultCapture.ResultCaptured += async (result) =>
            {
                ...
                ResultFound?.Invoke(
                    this,
                    new Views.Modals.SearchResultEventArgs
                    {
    ...
    )
    {
        ...
        try
        {
            ...
            var (configSuccess, configMessage, _, _, _) = await LoadConfigAsync(
                criteria.ConfigPath
            );
            if (!configSuccess)
            {
    ...
    private string BuildDetailsString(
        Motely.Filters.OuijaResult result,
