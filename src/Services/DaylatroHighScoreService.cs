...
namespace BalatroSeedOracle.Services
{
    ...
    public class DaylatroHighScoreService
    {
        ...
        private void SaveSubmissionDates()
        {
            try
            {
                var json = JsonSerializer.Serialize(_lastSubmissionDates, new JsonSerializerOptions { WriteIndented = true });
        ...
        /// </summary>
        public async Task<List<DaylatroHighScore>> FetchDaylatroScoresAsync(string? day = null, bool forceRefresh = false)
        {
            try
            {
                ...
                if (!forceRefresh && _webCache.TryGetValue(day, out var cached))
                {
                    ...
                    {
                        DebugLogger.Log("DaylatroHighScoreService", $"Returning cached scores for {day} (cached at {cached.fetchTime})");
        ...
        private List<DaylatroHighScore> ParseDaylatroHtml(string html)
        {
            ...
            try
            {
                ...
                while ((rowStart = tableContent.IndexOf("<tr", rowStart)) != -1)
                {
                    ...
                    if (cells.Count >= 3)
                    {
                        if (int.TryParse(cells[1], out var ante) && long.TryParse(cells[2].Replace(",", ""), out var bestHand))
                        {
                            scores.Add(new DaylatroHighScore
                            {
                                Player = cells[0],
                                Ante = ante,
                                Score = bestHand,
                            ...
                            DebugLogger.Log("DaylatroHighScoreService", $"Parsed score: {cells[0]} - Ante {ante} - {bestHand}");
                        }
                    }
        ...
        public async Task<(bool success, string message)> SubmitToDaylatroAsync(string initials, int ante, long score)
        {
            try
            {
                ...
                if (!CanSubmitToday())
                {
                    ...
                    return (false, $"You've already submitted today's score. You can submit again in {hoursUntilReset}h {minutesUntilReset}m (UTC midnight)");
                }
                
                // Form data format from actual site: day=2025-08-14&name=BSO&ante=2&bestHand=12287
                // Name must be 3 characters
                var playerName = initials.Length > 3 ? initials.Substring(0, 3).ToUpper() : initials.ToUpper().PadRight(3);
                var day = DateTime.UtcNow.ToString("yyyy-MM-dd");
                
                DebugLogger.Log("DaylatroHighScoreService", $"Submitting to Daylatro: day={day}, name={playerName}, ante={ante}, bestHand={score}");
                ...
                {
                    DebugLogger.Log("DaylatroHighScoreService", $"Successfully submitted score to Daylatro: {score} by {playerName}");
                ...
                {
                    DebugLogger.LogError("DaylatroHighScoreService", $"Failed to submit to Daylatro: {response.StatusCode}");
        ...
        private async Task<bool> SubmitToDaylatroAsync(DaylatroHighScore score)
        {
            try
            {
                // Form data format from actual site: day=2025-08-14&name=BSO&ante=2&bestHand=12287
                // Name must be 3 characters
                var playerName = score.Player.Length > 3 ? score.Player.Substring(0, 3).ToUpper() : score.Player.ToUpper().PadRight(3);
                var day = DateTime.UtcNow.ToString("yyyy-MM-dd");
                
                DebugLogger.Log("DaylatroHighScoreService", $"Submitting to Daylatro: day={day}, name={playerName}, ante={score.Ante}, bestHand={score.Score}");
                ...
                if (response.IsSuccessStatusCode)
                {
                    DebugLogger.Log("DaylatroHighScoreService", $"Successfully submitted score to Daylatro: {score.Score} by {score.Player}");
                    return true;
                }
                else
                {
