...
namespace BalatroSeedOracle.ViewModels
{
            ...
            BalatroSeedOracle.Helpers.IModalBackNavigable
    {
        ...
        public string[] SearchModeDisplayValues { get; } = new[] { "All Seeds", "Single Seed", "Word List", "DB List" };
        ...
        public object? CurrentTabContent =>
            SelectedTabIndex >= 0 && SelectedTabIndex < TabItems.Count ? TabItems[SelectedTabIndex].Content : null;
        ...
        private async Task StartSearchAsync()
        {
            try
            {
                ...
                if (SelectedSearchMode == SearchMode.SingleSeed && string.IsNullOrWhiteSpace(SeedInput))
                {
                ...
                ClearResults();
                AddConsoleMessage($"Starting search in {SearchModeDisplayValues[(int)SelectedSearchMode]} mode...");
                ...
                if (
                    IsSmartAutoMode
                    && (SelectedSearchMode == SearchMode.AllSeeds || SelectedSearchMode == SearchMode.WordList)
                )
                {
...
#else
                // Browser: Search functionality not available
                AddConsoleMessage("Search functionality is not available in browser mode.");
                return;
#endif
        ...
        private void OnSearchCompleted(object? sender, EventArgs e)
        {
            Avalonia.Threading.Dispatcher.UIThread.Post(async () =>
            {
                ...
                DebugLogger.Log("SearchModalViewModel", $"Search completed with {ResultsCount} results");
            });
        }
        ...
        private string GetSearchId()
        {
            ...
            var stake =
                string.IsNullOrEmpty(StakeSelection) || StakeSelection == "All Stakes" ? "White" : StakeSelection;
            ...
            switch (SelectedSearchMode)
            {
                case SearchMode.AllSeeds:
                    ...
                    if (ContinueFromLast && !string.IsNullOrEmpty(CurrentFilterPath))
                    {
                        ...
                        var savedState =
                            searchStateManager != null ? await searchStateManager.LoadSearchStateAsync(dbPath) : null;
                        if (savedState != null)
                        {
        ...
        private void OnProgressUpdated(object? sender, SearchProgress e)
        {
            try
            {
                ...
                if (SelectedSearchMode == SearchMode.AllSeeds && e.SeedsSearched > 0)
                {
                    ...
                    if (currentBatch > 0 && currentBatch % 10 == 0 && !string.IsNullOrEmpty(CurrentFilterPath))
                    {
        ...
        private void UpdateUIFromProgress(SearchProgress e)
        {
            Avalonia.Threading.Dispatcher.UIThread.Post(() =>
            {
                ...
                if (e.ResultsFound > LastKnownResultCount)
                {
                    ...
                    {
                        AddConsoleMessage($"Found {newSeedsCount} new seeds! Total: {e.ResultsFound}");
                    }
                    LastKnownResultCount = e.ResultsFound;
                }
            });
        ...
        private void LoadAvailableWordLists()
        {
            try
            {
                ...
                if (Directory.Exists(wordListDir))
                {
                    ...
                    DebugLogger.Log("SearchModalViewModel", $"Loaded {AvailableWordLists.Count} word lists");
                }
                else
                {
                    DebugLogger.Log("SearchModalViewModel", $"WordLists directory not found: {wordListDir}");
                }
            }
            catch (Exception ex)
            {
                DebugLogger.LogError("SearchModalViewModel", $"Failed to load wordlists: {ex.Message}");
            }
        }
        ...
        private void LoadAvailableDbLists()
        {
            try
            {
                ...
                if (Directory.Exists(searchResultsDir))
                {
                    ...
                    DebugLogger.Log("SearchModalViewModel", $"Loaded {AvailableDbLists.Count} DB lists");
                }
                else
                {
                    DebugLogger.Log("SearchModalViewModel", $"SearchResults directory not found: {searchResultsDir}");
                }
            }
            catch (Exception ex)
            {
                DebugLogger.LogError("SearchModalViewModel", $"Failed to load DB lists: {ex.Message}");
            }
        }
        ...
        private async Task LoadSavedProgressAsync()
        {
            try
            {
                if (string.IsNullOrEmpty(CurrentFilterPath))
                {
                    DebugLogger.Log("SearchModalViewModel", "Cannot load saved progress: No filter path");
                    return;
                }
            ...
            {
                DebugLogger.LogError("SearchModalViewModel", $"Failed to load saved progress: {ex.Message}");
                ProgressPercent = 0.0;
            }
        }
