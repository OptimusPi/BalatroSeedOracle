...
{
    private readonly IMotelySearchContext? _context; // Nullable for remote
    private readonly MotelyJsonConfig _config;
    private readonly string _searchId; // Store explicitly
    
    private bool _hasNewResults;

    /// <summary>
    /// Create an ActiveSearchContext wrapping a local Motely search context.
    /// </summary>
    ...
    /// <summary>
    /// Create an ActiveSearchContext for a remote search (no local context).
    /// </summary>
    ...
    public bool IsRunning => _context?.Status == MotelySearchStatus.Running || (_context == null); // Assume running if remote
    ...
    {
        if (_context == null) return new List<SearchResult>(); // Remote not impl yet
